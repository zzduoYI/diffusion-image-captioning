...
# hyperparameters
DEBUG = True
CONTINUE_TRAIN = False  # 接着某一个模型训练用的
BATCH_SIZE = 3
MAX_LENGTH = 60  # max text length
LEARNING_RATE = 1e-4
END_LEARNING_RATE = 5e-5  # learning rate is reduced to end_learning_rate


# 余弦退火（Cosine Annealing）的学习率调整策略
...
image_set = torch.load("DataSet/iux_test_image.pickle").to(device).detach()
text_set = torch.load("DataSet/iux_test_text.pickle").to(device).detach()

from spacy.lang.en import English
from collections import Counter
import itertools

captions = pd.read_csv("DataSet/caption.txt")["caption"]
nlp = English()
...
if TRAIN_EMBEDDING:
    ...
    ## 执行这里了
    tokenizer = DistilBertTokenizer.from_pretrained("HuggingFace/distilbert-base-uncased-local/",
                                                    local_files_only=True)
    VOCAB_SIZE = tokenizer.vocab_size

dataset = FlickrCLIPDataset(pd.read_csv("DataSet/caption_test.txt")["caption"],
                            pd.read_csv("DataSet/caption_test.txt")["image"], tokenizer)

if CONTINUE_TRAIN:
    val_set = torch.load(f"Results/{MODEL_NAME}.valset")
...
class DistilBertModel(nn.Module):
    def __init__(self, embedding=None, projection=None, config=None) -> None:
        ...
        if TRAIN_EMBEDDING:
            ...
            ## copy.deepcopy()创建一个全新无干扰的; requires_grad_(False)是因为嵌入层可能是预训练好的
    ...
    def forward(self, x, image_clip, text_clip, mask, concat_mask):
        ...
        assert image_clip.shape == text_clip.shape == (sample_batch_multi, 2, 512)
        ...
        if CLIP_ADDING_METHOD == "concat":
            ...
            classifier_guided_mask = torch.hstack(
                [mask, torch.tensor([1, 1], device=device).repeat(sample_batch_multi, 2)])
            non_classifier_mask = torch.hstack(
                [mask, torch.tensor([1, 0], device=device).repeat(sample_batch_multi, 2)])
            ...
            x = x + self.segment_embedding(torch.tensor([0] * MAX_LENGTH + [1] * 4, device=device))
...
if TRAIN_EMBEDDING:
    ...
    origin = DistilBertForMaskedLM.from_pretrained("HuggingFace/distilbert-base-uncased-local", local_files_only=True).to(
        device)
    configuration = DistilBertConfig()
...
def loss(model, x_t, x_1, x_tgt, x_0, image_clip, text_clip, mask, idx, loss_func):
    ...
    assert image_clip.shape == text_clip.shape == (BATCH_SIZE, 2, 512)
...
def train_func(model, trainer, x, train=True):
    ...
    if train:
        trainer.zero_grad()
    x_t_loss, x_1_loss, prob_loss = loss(model, x_t, x_1, x_tgt, x_0, x["image_clip"], x["text_clip"],
                                         x["attention_mask"], x["input_ids"], LOSS_FUNC)
...
for epoch in range(EPOCH_NUM):
    ...
    if val_x_t + val_x_1 + val_prob > EARLY_STOP_RATIO * acc_l / len(train_loader):
        if not early_stopped:
            ...
            torch.save(model.cpu(), f"Results/{MODEL_NAME}.pickle")
...
with torch.no_grad():
    ...
    image_clip = sample["image_clip"][None, :]
    text_clip = sample["text_clip"][None, :]
...
with torch.no_grad():
    for j, x in enumerate(val_loader):
        ...
        for i in range(5):
            ...
            out, restored = model(restored[:, :MAX_LENGTH, :], x["image_clip"],
                                  torch.zeros_like(x["image_clip"], device=device),
                                  torch.ones((x["input_ids"].shape[0], MAX_LENGTH), device=device),
                                  torch.tensor([1, 0], device=device).repeat(x["attention_mask"].shape[0], 1))
...
torch.save(val_set, f"Results/{MODEL_NAME}.valset")
